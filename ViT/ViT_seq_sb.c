...
static void init_kernel(Network* networks) {
    ...
    for (int i = 0; i < 2; i++) {
        ctx.enc_buf[i] = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE, sizeof(float) * batch_size * enc_size, NULL, &err); CHECK_ERROR(err);
    }

    ctx.img_buf = clCreateBuffer(ctx.context, CL_MEM_READ_ONLY, sizeof(float) * batch_size * in_chans * img_size * img_size, NULL, &err); CHECK_ERROR(err);
    ctx.patch_buf = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE, sizeof(float) * batch_size * embed_dim * num_patches, NULL, &err); CHECK_ERROR(err);
    ctx. prepare_buf = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE, sizeof(float) * batch_size * embed_dim * tokens, NULL, &err); CHECK_ERROR(err);
    ctx.cls_tokens = clCreateBuffer(ctx.context, CL_MEM_READ_ONLY, sizeof(float) * batch_size * embed_dim, NULL, &err);
    ctx.cls_output = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE, sizeof(float) * batch_size * num_classes, NULL, &err);
...
void ViT_seq_sb(ImageData* image, Network* networks, float** probabilities) {
    ...
    for (int i = 0; i < image->n; i += batch_size) {
        ...
        for (int j = 0; j < current_batch_size; j++) {
            ...
            err = clEnqueueWriteBuffer(ctx.input_queue, ctx.img_buf, CL_FALSE, image_bytes * j,
        ...
        // Patch Embedding
        Conv2d(ctx.img_buf, ctx.patch_buf, networks[1], networks[2]);

        // class token                              ¦¤
        // class token prepended + patch tokens     ¦¢- in one kernel
        // positional encoding                      ¦¥
        prepare_input(ctx.patch_buf, networks[0].buffer, networks[3].buffer, ctx.prepare_buf);

        Encoder(ctx.prepare_buf, ctx.enc_buf[0],
        ...
        Encoder(ctx.enc_buf[0], ctx.enc_buf[1],
        ...
        Encoder(ctx.enc_buf[1], ctx.enc_buf[0],
        ...
        err = clSetKernelArg(ctx.extract_cls_kernel, 1, sizeof(cl_mem), &ctx.cls_tokens); CHECK_ERROR(err);
        ...
        err = clSetKernelArg(ctx.softmax_kernel, 0, sizeof(cl_mem), &ctx.cls_output); CHECK_ERROR(err);
...
static void release_kernel() {
	clReleaseMemObject(ctx.qkv_buf);
	clReleaseMemObject(ctx.attn_buf);
	clReleaseMemObject(ctx.score_buf);

    clReleaseMemObject(ctx.fc1_buf);

	clReleaseMemObject(ctx.img_buf);
	clReleaseMemObject(ctx.patch_buf);
	clReleaseMemObject(ctx.prepare_buf);
	clReleaseMemObject(ctx.cls_tokens);
	clReleaseMemObject(ctx.cls_output);

	for (int i = 0; i < 2; i++) {
		clReleaseMemObject(ctx.enc_buf[i]);
	}
