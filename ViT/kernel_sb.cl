...
__kernel void attention_score(
    ...
    int q_base = i * qkv_dim + head_offset;
    int k_base = j * qkv_dim + embed_dim + head_offset;
    ...
    scores[i * tokens + j] = score * scale;
}

__kernel void softmax(__global float* scores, const int tokens) {
...
__kernel void context (
    ...
    if (i >= tokens || d >= head_dim) return;

    float sum = 0.0f;

    for (int j = 0; j < tokens; j++) {
        float s = scores[i * tokens + j];
        float v = QKV[j * qkv_dim + (2 * embed_dim) + head_offset + d];

        sum += s * v;
    }

    int output_idx = i * embed_dim + head_offset + d;
    attn_output[output_idx] = sum;
}
...
__kernel void conv2d (
    ...
    if (oc >= embed_dim || oh >= output_size || ow >= output_size) return;
    float sum = bias[oc];
    ...
    int patch_index = oh * output_size + ow;
    int out_idx = patch_index * embed_dim + oc; 
